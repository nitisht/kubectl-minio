...
func NewCmdMinIO(streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:          "minio",
		Short:        "manipulate, inspect and troubleshoot MinIO Operator Specific CRDs.",
		Long:         `MinIO Operator plugin can be used to manipulate, inspect and troubleshoot MinIO Operator Specific CRDs.`,
		SilenceUsage: true,
		Example: `  # Install MinIO Operator from the official GitHub repo.
		kubectl minio init [options] [flags]`,
	}

	cmd.AddCommand(newInitCmd(cmd.OutOrStdout(), cmd.ErrOrStderr(), nil))
